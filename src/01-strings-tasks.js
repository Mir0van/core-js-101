 ...
 */
const extractNameFromTemplate = (str) => str.slice(7, -1);
...
const getRectangleString = (width, height) => {
  const firstSymbol = '┌';
  const lastSymbol = '┐';
  const firstDownSymbol = '└';
  const lastDownSymbol = '┘';
  ...
  for (let i = 0; i < height; i += 1) {
    if (i === 0) {
      str += firstSymbol + horizontalSymbol.repeat(width - 2) + lastSymbol + breakSymbol;
    } else if (i === height - 1) {
      str += firstDownSymbol + horizontalSymbol.repeat(width - 2) + lastDownSymbol + breakSymbol;
    } else {
      str += verticalSymbol + spaceSymbol.repeat(width - 2) + verticalSymbol + breakSymbol;
    }
  }
...
const encodeToRot13 = (str) => str.replace(/[a-zA-Z]/g, (char) => {
  const startCharCode = char <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
